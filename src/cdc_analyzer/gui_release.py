    ...
    if language == "zh_CN":
        ...
        <ul>
          ...
          <ul>
            ...
            <li><b>稳定时间：</b>信号经阶跃/过冲/下冲后，进入目标稳态允许带并持续保持所需时间。目标电流为阶跃终值；稳定带默认目标值 ± 2%（可配置）。判据是“进入 + 持续保持”，不是“第一次进入”：在带内连续保持默认 10 ms（连续保持时长可配置）才认定为稳定。若波形第一次经过目标值后继续掉到 0.7 A，绝不能算已稳定。稳定时间 = 稳定时刻 − t(I₁₀%)。</li>
            <li><b>恢复时间：</b>仅用于瞬态跌落-恢复型波形（F₀→F_min→F₀）。事件前基准阻尼力取触发前阻尼力中位数；恢复带 = max(2%·基准值, 3σ 噪声, 绝对下限)。力从最低点恢复后，第一次重新进入恢复带并连续保持 10 ms 的时刻为恢复时刻。恢复时间 = 恢复时刻 − t(F_min)，即从异常最低点恢复到重新稳定所需时间。</li>
            <li><b>总瞬态时间：</b>= 恢复时刻 − t(I₁₀%)，从电流开始有效变化到阻尼力完全恢复的总持续时间。</li>
            <li><b>三者关系</b>（示例 t(I₁₀%)=0、t(F_min)=8 ms、恢复时刻=24 ms）：到最低点 8 ms（跌落形成过程）；恢复时间 = 24−8 = 16 ms（恢复过程）；总瞬态时间 = 24 ms（整个异常瞬态）。电流波形同构：极值时刻 t(I_min)、电流恢复时间 = 电流稳定时刻 − t(I_min)、电流稳定时间 = 电流稳定时刻 − t(I₁₀%)，且稳定时间 = 到极值时间 + 恢复时间。</li>
            <li><b>适用性：</b>正常单调阶跃（F₀→F∞）一般只需稳定时间，不需要恢复时间；瞬态跌落-恢复型波形（F₀→F_min→F₀）用恢复时间 + 总瞬态时间比传统 t₆₃% / t₉₀% 更有意义。</li>
            <li><b>默认稳定判据：</b>不要求“此后永远不出带”（测试数据可能很长），采用固定连续保持时长——连续 10 ms 在带内即认定稳定 / 已恢复。电流稳定带默认 ±2% 目标值；力恢复带默认 max(±2% 基准, ±3σ 噪声)；连续保持时长默认 10 ms。全部可配置；报告自动打印算法定义，客户可直接复核“这个 24 ms 怎么来的”。</li>
          </ul></li>
