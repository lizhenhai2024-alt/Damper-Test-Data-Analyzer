...
def _release_help_html(language: str) -> str:
    ...
    if language == "zh_CN":
        ...
        <ul>
          ...
          <li><b>双 Y 轴阻尼力响应：</b>X 轴为时间，左侧蓝色轴与曲线表示电流，右侧红色轴与曲线表示阻尼力。两条竖虚线标记电流 I₁₀% 和阻尼力 F₉₀% 的线性插值时刻；电流及阻尼力曲线均显示对应的两个交点。主响应时间为 t(F₉₀%) − t(I₁₀%)，与三联图一致。电流 I₁₀%—I₉₀% 时间仅作为辅助结果导出。</li>
        ...
        <ul>
          ...
          <li><b>Dual-axis damping response:</b>Time is the X axis. The blue left axis and curve show current; the red right axis and curve show damping force. Two vertical dashed guides mark the linearly interpolated current I₁₀% and force F₉₀% times, with two intersections on each curve. The primary result is t(F₉₀%) − t(I₁₀%), matching the three-panel plot. Current I₁₀%–I₉₀% remains an auxiliary exported metric.</li>
