...
- 响应时间分析新增“计算电流过冲”勾选项（默认开启），对电流过冲（上升超过终值）或下冲（下降低于终值）引起的瞬态力跌落进行识别；图中在电流曲线上标注 Iₘₐₓ= / Iₘᵢₙ= 电流极值（过冲率 >10% 时附标注）与 I₁₀₀%= 电流满幅值。
- 识别为“瞬态跌落-恢复型响应（Dip & Recovery）”时，稳态力差不足，传统 t₆₃%/t₉₀% 不适用，图中明确标注并在计时上做门控处理，避免把瞬态跌落误判为真实响应。

### V0.9.5 稳定时间与恢复时间（算法定义，严格区分勿混用）

- **响应起点**：t₀ = t(I₁₀%)，电流触发交点时刻，所有时间差均以此为基准或显式起算。
- **稳定时间 Settling Time**：信号经阶跃/过冲/下冲后，进入目标稳态允许带并持续保持所需时间。目标电流 I_tar 为阶跃终值；稳定带默认 I_tar ± 2%（可配置）。判据是“进入 + 持续保持”，不是“第一次进入”：在带内连续保持默认 10 ms（dwell，可配置）才认定为稳定；波形第一次经过目标值后继续掉到 0.7 A 绝不能算已稳定。T_settle,I = t_settle − t(I₁₀%)。
- **恢复时间 Recovery Time**：仅用于 Dip & Recovery 波形（F₀→F_min→F₀）。事件前基准阻尼力 F_base = median(F_pre)；恢复带 Band_F = max(2%·|F_base|, 3σ_F, Band_abs)。力从最低点恢复后，第一次重新进入恢复带并连续保持 10 ms 的时刻为 t_recover。T_recovery = t_recover − t(F_min)。
- **总瞬态时间 Total Transient Time**：T_transient = t_recover − t(I₁₀%)，从电流开始有效变化到阻尼力完全恢复的总持续时间。
- **三者关系**（示例 t(I₁₀%)=0、t(F_min)=8 ms、t_recover=24 ms）：到最低点 T_to_min = 8 ms（跌落形成）；T_recovery = 24−8 = 16 ms（恢复）；T_transient = 24 ms（整个异常瞬态）。电流波形同构：t(I_min)、T_I,recovery = t(I,settle) − t(I_min)、T_I,settle = t(I,settle) − t(I₁₀%)，且 T_settling = T_to_extreme + T_recovery。
- **适用性**：正常单调阶跃（F₀→F∞）一般只需稳定时间；Dip & Recovery 波形（F₀→F_min→F₀）用恢复时间 + 总瞬态时间比传统 t₆₃%/t₉₀% 更有意义。
- **默认稳定判据**：不要求“此后永远不出带”（测试数据可能很长），采用固定 dwell——连续 10 ms 在带内即认定 stable/recovered。电流稳定带默认 ±2% 目标值；力恢复带默认 max(±2% 基准, ±3σ 噪声)；dwell 默认 10 ms，全部可配置。报告自动打印算法定义，客户可直接复核“这个 24 ms 怎么来的”。
